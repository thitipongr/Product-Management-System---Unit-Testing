...
productsRouter.route("").get((req, res) => {
  ...
  con.query(findWithPage_sql, (err, result) => {
    ...
    if (result.length === 0) return res.send("There are no products here."); // หากไม่มี ก็แสดงข้อความแจ้ง
...
productsRouter.route("/:id").get((req, res) => {
  ...
  con.query(fineById, (err, result) => {
    ...
    if (result.length === 0) return res.send("Product not found."); // หากไม่มี ก็แสดงข้อความแจ้ง
